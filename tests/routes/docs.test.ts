...
describe('GET /openapi.json', () => {
  ...
  it('omits admin routes', async () => {
    const res = await request(app).get('/openapi.json');
    const paths = res.body.paths as Record<string, unknown>;
    expect(paths['/api/admin/status']).toBeUndefined();
    expect(paths['/api/admin/pause']).toBeUndefined();
    expect(paths['/api/admin/reindex']).toBeUndefined();
    expect(paths['/api/admin/api-keys']).toBeUndefined();
  });

  it('omits DLQ routes', async () => {
    const res = await request(app).get('/openapi.json');
    const paths = res.body.paths as Record<string, unknown>;
    expect(paths['/admin/dlq']).toBeUndefined();
    expect(paths['/admin/dlq/{id}']).toBeUndefined();
  });

  it('omits indexer routes', async () => {
    const res = await request(app).get('/openapi.json');
    const paths = res.body.paths as Record<string, unknown>;
    expect(paths['/internal/indexer/contract-events']).toBeUndefined();
    expect(paths['/internal/indexer/events']).toBeUndefined();
    expect(paths['/internal/indexer/events/replay']).toBeUndefined();
  });

  it('omits webhook routes', async () => {
    const res = await request(app).get('/openapi.json');
    const paths = res.body.paths as Record<string, unknown>;
    expect(paths['/internal/webhooks/receive']).toBeUndefined();
    expect(paths['/internal/webhooks/queue']).toBeUndefined();
  });
...
// ── ContractEventSchema — OpenAPI spec tests ──────────────────────────────────
  ...
  describe('Negative testing for docs endpoints', () => {
    it('returns 404 for POST /openapi.json (malformed method)', async () => {
      const res = await request(app).post('/openapi.json').send({});
      expect(res.status).toBe(404);
    });

    it('returns 404 for POST /docs (malformed method)', async () => {
      const res = await request(app).post('/docs').send({});
      expect(res.status).toBe(404);
    });
    
    it('ignores dummy credentials for /openapi.json', async () => {
      const res = await request(app).get('/openapi.json').set('Authorization', 'Bearer dummy');
      expect(res.status).toBe(200);
    });

    it('ignores dummy credentials for /docs/', async () => {
      const res = await request(app).get('/docs/').set('Authorization', 'Bearer dummy');
      expect(res.status).toBe(200);
    });
  });
