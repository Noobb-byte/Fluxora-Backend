...
**Query Builder & Bounded Result Sets Guarantee:**

As defense-in-depth against service memory exhaustion and latency cliffs, all database read paths
for stream collections are generated by `StreamQueryBuilder` (`src/db/queries/streams.ts`):
- **Explicit Limit Required**: Every query path must supply an explicit limit. Calling the builder without a limit fails at compile-time and throws `UnboundedStreamQueryError` at runtime.
- **Default Limit**: Documented in code as `DEFAULT_STREAM_LIMIT = 50`, matching the API default page size.
- **Server-Side Cap Enforcement**: Any limit exceeding `MAX_STREAM_LIMIT = 100` is automatically clamped to 100 server-side inside the builder, regardless of caller or route input.
- **Mandatory LIMIT Clause**: The builder verifies that every emitted SQL string carries a valid `LIMIT` clause before passing it to PostgreSQL.
